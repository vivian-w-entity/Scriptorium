...
const EditBlogPost = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      // Fetch post data from your API using the post ID
      const fetchPost = async () => {
        try {
          ...
          if (response.ok) {
            ...
            setTitle(data.title)
            setDesc(data.desc)
            setTagsInput((data.postTags.map((tag: { name: string }) => tag.name)).join())
            setTags((data.postTags.map((tag: { name: string }) => tag.name)).join())
            setSelectedTemplateId(data.postTemplates[0])
          } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!token || getUserId() !== post.userId) {
      setError("You must be the owner of a post to edit it.");
      setIsSubmitting(false);
      return;
    }

    try {
      const response = await fetch(`/api/blogposts/${post.id}/edit`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        setError(errorData.error || "Failed to edit blog post.");
      } else {
        const data = await response.json();
        console.log("Blog post edited:", data);
  ...
  const handleDelete = async (e: FormEvent) => {
    ...
    if (!token || getUserId() !== post.userId) {
      setError("You must be the owner of a post to delete it.");
      setIsSubmitting(false);
      return;
    }
  ...
  return (
    <div className="container">
      ...
      {!isAuthenticated && (
        <p className="error">You must be logged in to edit a blog post.</p>
      )}
